...
function HashTable() {
  ...
  this.hash = function (clave) {
    ...
    for (let i = 0; i < clave.length; i++) {
      valorHash += clave.charCodeAt(i);
      
    }
    return valorHash % this.numBuckets;
  };
  ...
  this.set = function (clave, valor) {
    ...
    const elementos = this.tabla[indice];
  ...
  this.get = function (clave) {
    ...
    let indice = this.hash(clave);
    if (this.tabla[indice]) {
      for (const element of this.tabla[indice]) {
  ...
  this.hasKey = function (clave) {
    ...
    let indice = this.hash(clave);
    if (this.tabla[indice]) {
      for (const elemento of this.tabla[indice]) {
        if (elemento.clave.toLowerCase() === clave.toLowerCase()) {
          return true;
        }
      }
    }
